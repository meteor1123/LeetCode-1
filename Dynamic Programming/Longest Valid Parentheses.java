...
/*
    ...
    2. 遇到')'，一律不进栈，此时判断stack是否为空如果为空
       1） 说明之前已经将'('匹配完出栈，已经没有'('可以进行匹配，所以需要将start的位置 赋值为i+ 1，跳过这个')'
    3. 如果栈非空， pop 一个'('和这个')'进行匹配， 匹配完成后判断栈是否为空，
        如果空  maxLen = Math.max(maxLen, i - start + 1) 
        栈非空  maxLen = Math.max(maxLen, i - stack.pop())
*/
